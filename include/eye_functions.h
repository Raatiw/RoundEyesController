...
void initEyes(void)
{
  USBSerial.println("Initialise eye objects");

  // Initialise eye objects based on eyeInfo list in config.h:
  for (uint8_t e = 0; e < NUM_EYES; e++) {
    USBSerial.print("Create display #"); USBSerial.println(e);
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  if (!(++frames & 255)) { // Every 256 frames...
    ...
    if (elapsed) USBSerial.println((uint16_t)(frames / elapsed)); // Print FPS
  }
